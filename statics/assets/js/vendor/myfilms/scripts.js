$(document).ready(function(){
	...
	const nowPlayingURL = apiBaseURL + 'movie/now_playing?language=es-ES&api_key=' + apiKey;

	function getHtmlCard(selection,result,i){
		// w300 is how wide it is
		var mid = result.id;
		// mid = movie ID
		var thisMovieUrl = apiBaseURL + 'movie/' + mid + '/videos?language=es-ES&api_key=' + apiKey;
		// console.log(i)

		$.getJSON(thisMovieUrl, function (movieKey) {

			var poster = imageBaseUrl + 'w300' + result.poster_path;
			...
			var youtubeKey = movieKey.results.length ? movieKey.results[0].key : 'xxxxx';
			...
			$(selection).append(nowPlayingHTML);
		})
	}
	...
	function getNowPlayingData(){
		$.getJSON(nowPlayingURL, function(nowPlayingData){
			...
			for(let i = 0; i < nowPlayingData.results.length; i++){
				getHtmlCard('#movie-grid',nowPlayingData.results[i],i);
			}
	...
	function getMoviesByGenre(genre_id){
		const getMoviesByGenreURL = apiBaseURL + 'genre/' + genre_id + '/movies?language=es-ES&api_key=' + apiKey + '&language=es-ES&include_adult=false&sort_by=created_at.asc';
	...
	async function searchMovies(searchterm){
		...
		const searchMovieURL = apiBaseURL + 'search/movie?language=es-ES&api_key=' + apiKey + '&page=1&include_adult=true&query=' + searchterm;
			// console.log(searchMovieURL);
		$.getJSON(searchMovieURL,async function(movieSearchResults){
			// console.log(movieSearchResults);
			for (let i = 0; i<movieSearchResults.results.length; i++){
				var mid = movieSearchResults.results[i].id;
				var thisMovieUrl = apiBaseURL+'movie/'+mid+'/videos?language=es-ES&api_key=' + apiKey;

				$.getJSON(thisMovieUrl, async function(movieKey){
					// console.log(movieKey)
